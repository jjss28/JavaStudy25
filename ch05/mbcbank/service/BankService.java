...
public class BankService {// 서비스 계층은 CRUD 메서드 위주로 생성된다.
	// 필드

	// 생성자

	// 메서드 (부메뉴, 계좌 생성, 입금, 출금, 삭제, 이체 등...)

	public void menu(Scanner inputInt, Scanner inputStr, HanaAccountDTO[] hanaBank, NhAccountDTO[] nhBank, WooriAccountDTO[] wooriBank) {
		boolean subRun = true;
		while (subRun) {
			System.out.println("-----------------------------");
			...
			case "1":
				...
				createAccount(inputStr, inputStr, hanaBank);
				break;
			case "2":
				System.out.println("=====계좌 목록보기 메서드로 진입합니다=====");
				accountLists(inputStr, inputStr, hanaBank);
				break;
			case "3":
				System.out.println("=====계좌 입금 메서드로 진입합니다=====");
				deposit(inputStr, inputStr, hanaBank);
				break;
			case "4":
				System.out.println("=====계좌 출금 메서드로 진입합니다=====");
				withdraw(inputStr, inputStr, hanaBank);
				break;
			case "5":
	...
	private void withdraw(Scanner inputStr, Scanner inputInt, HanaAccountDTO[] hanaBank) {
		// 계좌번호를 입력하고 출금액을 입력하면 계좌번호를 찾아서 계좌잔액을 감소 시킨다.
	...
	private void createAccount(Scanner inputStr, Scanner inputInt, HanaAccountDTO[] hanaBank) {
		...
		} // 하나은행 배열을 0번에서부터 반복 코드
		System.out.println("계좌 생성이 정상적으로 처리 되었습니다.");
